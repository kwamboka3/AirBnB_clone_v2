    ...
    def do_create(self, args):
        ...
        if len(args.split()) == 1:
            ...
            if class_name in HBNBCommand.classes:
                all_par_list = args.split()[1:]
                new_list = []
                for par in all_par_list:
                    if par.find("=") > 0:
                        new_list.extend(par.split("="))
                    else:
                        pass
                for i in range(0, len(new_list)):
                    if (i % 2) != 0: 
                        if new_list[i][0] == '"' and new_list[i][-1] == '"':
                ...
                for i in range(0, len(new_list)):
                    if (i % 2) == 0:
                        par_dict.update({new_list[i]: new_list[i+1]})
                new_instance = HBNBCommand.classes[class_name]()
                new_instance.__dict__.update(par_dict)
                new_instance.save()
                print(new_instance.id)
